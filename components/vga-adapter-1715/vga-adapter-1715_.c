...
#define PIX_STATUS_START 640*288
...
void IRAM_ATTR app_main(void)
{
	...
	// uint8_t* bufmodifier = heap_caps_malloc(640, MALLOC_CAP_8BIT|MALLOC_CAP_INTERNAL); // lookup table für die Korrektur der Samplefrequenz zu CRT Frequenz

	// Timer-Interrupt (Sheduler) deaktivieren
	REG_WRITE(INTERRUPT_CORE1_SYSTIMER_TARGET1_INT_MAP_REG, 6);

	// Lookup Tabelle füllen
	// for (int i=0;i<640;i++) {
	// 	bufmodifier[i] = 3;
	// 	// if (i % (int) 16 == 0)
	// 	// 	bufmodifier[i]--;
	...
	while (1)
	{
		...
		*/
		...
		if (sync>0)
		{
			...
			// for (int i=0;i<40;i++) // wir machen 16 Pixel pro Durchlauf, also 40*16 = 640 Pixel
			// {
			// 	*vgapos = COLORS[((*bufpos)|(*(bufpos+1))) & 3];
			// 	vgapos++;
			// 	bufpos+=3;
